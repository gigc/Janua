...
void TgcDX11Texture::internalCreateFromFile()
{
	//Create texture from file
	ID3D11Device* device = ((TgcDX11Renderer*)GuiController::Instance->renderer)->device;
	HRESULT result = D3DX11CreateShaderResourceViewFromFile(device, this->filePath.c_str(), NULL, NULL, &(this->dxTexture), NULL);
	if(FAILED(result))
	{
...
void TgcDX11Texture::internalDispose()
{
	if(this->dxTexture)
	{
...
TextureFormat::Format TgcDX11Texture::getTextureFormat(DXGI_FORMAT dxFormat)
{
	switch(dxFormat)
	{
	//Common case
